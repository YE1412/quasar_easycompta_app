...
declare module "@capacitor/cli" {
	export interface CapacitorConfig {
	    /**
	     * The unique identifier of your backend app.
	     * 
	     * This is also known as the Bundle ID in iOS and the Application ID in
	     * Android. It must be in reverse domain notation, generally
		 ...
		 * 
		 * This should be what you'd see in the App Store, but can be change after
		...
		/**
		 * Whether to compy the Capacitor runtime bundle or not.
		 * 
		 * If your app is not using a bundler, set it to `true`, then Capacitor
		 ...
		 * willl send statements to the log system. This applies to log statements in
		 ...
		 * Xcode and Android Studio windows but can leak information on devise if enabled
		 * in release builds.
		 * 
		 * 'none' = logs are never produced
		 * 'debug' = logs are produces in debug builds but not in production builds
		...
		/**
		 * User agent of CapacitorWeb View.
		 * 
		 * @since 1.4.0
		 */
		...
		android?: {
			...
			/**
			 * String to appand to the original user agent of Capacitor Web View for Android.
			 * 
			 ...
			 * 
			 * This is automatically enbled during development.
			 * 
			 * @since 1.0.0
			 * @default false
			 */
			webContentDebuggingEnabled?: boolean;
			 ...
			 * 
			 * If the device uses a lower WebView version, an error message wil be shown on Logcat.
		...
		ios?: {
			...
			/**
			 * String to append to the original user agent of Cacitor Web View fir iOS.
			 * 
			 * Overrides global `appendUserAgent` option.
			 * 
			 * @since 1.4.0
		...
		server?: {
			 ...
			 * 
			 * [Can't be set to schemes that the WKWebView already handles, such as http or https](https://developer.apple.com/documentation/webkit/wkwebviewconfiguration/287566-seturlschemehandler)
			 ...
			 * 
			 * **This is nor intended for use in production**
			 * 
			 * @since 1.0.0
			 */
			url?: string;
		...
		cordova?: {
			 ...
			 * If not provides, a single <access origin="*" /> tag gets included.
